...
module DockerizeRails
  class DockerfileGeneratorTest < Minitest::Test
    ...
    # New test: Ensure Dockerfile correctly copies .env file if it exists
    def test_generate_dockerfile_copies_env_file_if_exists
      File.write(File.join(@tmp_dir, '.env'), "MY_ENV_VAR=some_value")
      DockerizeRails::DockerfileGenerator.generate(@tmp_dir, :rails)
      content = File.read(File.join(@tmp_dir, 'Dockerfile'))

      assert_includes content, 'COPY .env .env'
    end

    # New test: Ensure Dockerfile does not copy .env file if it does not exist
    def test_generate_dockerfile_does_not_copy_env_file_if_not_exists
      DockerizeRails::DockerfileGenerator.generate(@tmp_dir, :rails)
      content = File.read(File.join(@tmp_dir, 'Dockerfile'))

      refute_includes content, 'COPY .env .env'
    end
  end
end
